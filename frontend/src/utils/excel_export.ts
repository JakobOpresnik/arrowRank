...
const thinLine: Partial<ExcelJS.Border> = { style: 'thin', color: { argb: BLACK } };
const mediumLine: Partial<ExcelJS.Border> = { style: 'medium', color: { argb: BLACK } };
...
function computeTotal(archer: ArcherExtended): number | '' {
  ...
  return scoreKeys.reduce<number>(
    (sum, k) => sum + ((archer[`score${k}` as keyof Archer] as number) ?? 0) * k,
    0,
  );
}
...
const exportTableToExcel = async (
  ...
  if (competition) {
    const orgLine = competition.organizer
      ? `Društvo ${competition.organizer}`
      : `Društvo ${competition.name}`;
    ...
    eventCell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
    ws.mergeCells(r, HDR_TEXT_START, r, HDR_TEXT_END);
  ...
  if (ptlBuffer) {
    ...
    ws.addImage(id, { tl: { col: TOTAL_COLS - 1.7, row: imgTlRow }, br: { col: TOTAL_COLS, row: imgBrRow }, editAs: 'oneCell' } as any);
  }
  ...
  if (competition?.logo_url) {
    ...
    if (orgBuffer) {
      ...
      const id = wb.addImage({ buffer: orgBuffer, extension: ext as 'png' | 'jpeg' | 'gif' });
      // Use fixed pixel size so the organizer logo isn't distorted regardless of column widths
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ws.addImage(id, { tl: { col: 0.1, row: imgTlRow }, ext: { width: 110, height: 80 } } as any);
    }
  }
  ...
  rezCell.font = { name: 'Calibri', size: 22, bold: false, color: { argb: BLACK } };
  rezCell.fill = whiteFill;
  rezCell.alignment = { horizontal: 'center', vertical: 'middle' };
  rezCell.border = { top: mediumLine, bottom: mediumLine, left: mediumLine, right: mediumLine };
  ...
  const colHeaders: (string | number)[] = ['Št.', 'Ime in priimek', 'Klub', 'Rezultat', ...scoreKeys];

  for (const [label, groupArchers] of groups) {
    // Category header
    const catCell = ws.getCell(r, 1);
    catCell.value = label;
    catCell.font = { name: 'Calibri', size: 11, bold: true, color: { argb: BLACK } };
    ...
    colHeaders.forEach((header, ci) => {
      ...
      cell.value = header;
      cell.font = { name: 'Calibri', size: 11, bold: col !== COL_RANK, color: { argb: BLACK } };
      cell.fill = whiteFill;
      cell.alignment = { horizontal: isNumeric ? 'center' : 'left', vertical: 'middle' };
    ...
    ranked.forEach((archer, idx) => {
      ...
      const setCell = (col: number, value: string | number, align: 'left' | 'center' = 'left', bold = false) => {
        const cell = ws.getCell(r, col);
        cell.value = value;
  ...
  const filename = competition ? `${competition.name}_rezultati.xlsx` : 'rezultati.xlsx';
